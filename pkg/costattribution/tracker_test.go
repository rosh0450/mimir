...
func Test_GetKeyValues(t *testing.T) {
	...
	keyVal1 := cat.getKeyValues(labels.FromStrings("department", "foo", "service", "bar"), 1, 0)
	assert.Equal(t, []string{"foo", "bar", "user3"}, keyVal1, "First call, expecting values as-is")

	keyVal2 := cat.getKeyValues(labels.FromStrings("department", "foo"), 3, 0)
	assert.Equal(t, []string{"foo", "__missing__", "user3"}, keyVal2, "Service missing, should return '__missing__'")

	keyVal3 := cat.getKeyValues(labels.FromStrings("department", "foo", "service", "baz", "team", "a"), 4, 0)
	assert.Equal(t, []string{"__overflow__", "__overflow__", "user3"}, keyVal3, "Overflow state expected")

	keyVal4 := cat.getKeyValues(labels.FromStrings("department", "foo", "service", "bar"), 5, 0)
...
func Test_Overflow(t *testing.T) {
	...
	assert.False(t, cat.overflow(stream1, []string{"foo", "bar", "user1"}, 1, 0), "First observation, should not overflow")
	assert.False(t, cat.overflow(stream2, []string{"bar", "baz", "user1"}, 2, 0), "Second observation, should not overflow")
	assert.True(t, cat.overflow(stream3, []string{"baz", "foo", "user1"}, 3, 0), "Third observation didn't seen before, should overflow")
	assert.True(t, cat.overflow(stream3, []string{"baz", "foo", "user1"}, 4, 0), "Fourth observation, should stay overflow")
